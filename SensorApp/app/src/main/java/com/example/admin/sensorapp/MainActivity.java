    ...
    public void onSensorChanged(SensorEvent event) {
        try {
           ...
           // if(threshold(event.values[0]) || threshold(event.values[1]) || threshold(event.values[2])) {
                String line = upScale(event.values[0]) + " " + upScale(event.values[1]) + " " + upScale(event.values[2]) + " "+ System.currentTimeMillis();
                ...
                if (output) {
                    textView.setText(" X: " + upScale(event.values[0]) +
                            "\n Y: " + upScale(event.values[1]) +
                            "\n Z: " + upScale(event.values[2]) +
                            "\n Recording : ON " +
